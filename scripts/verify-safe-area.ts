...
assert.equal(resolveSafeAreaTop({ measured: 0, cached: 47, keyboardOpen: true, viewportOffsetTop: 47 }).top, 0)
...
assert.match(css, /height:\s*calc\(var\(--app-height, 100dvh\) - var\(--titlebar-height\)\)/)
assert.match(css, /:root\[data-keyboard-open="true"\] \.workspace/)
assert.match(css, /:root\[data-keyboard-open="true"\] :is\(aside, \.source\)/)
