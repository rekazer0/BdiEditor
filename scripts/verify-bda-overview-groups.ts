...
assert.match(main, /\["imageStyles",\s*"样式配置",\s*"图片样式"[\s\S]*?\["textStyles",\s*"样式配置",\s*"文字样式"[\s\S]*?\["colorStyles",\s*"样式配置",\s*"颜色样式"/, "图片、文字和颜色样式应分别显示对应的 appearance 样式片段")
...
assert.match(main, /path: bdaAppearanceStylePath\(group\)[\s\S]{0,160}?navMode: "style"/, "appearance 样式片段应复用样式列表与详情检查器")
...
assert.match(main, /\["皮肤信息", "样式配置", "资源配置", "面板样式"/, "样式与资源配置应位于面板样式上方")
