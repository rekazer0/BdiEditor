...
- 移动端属性检查器改为左侧面板填满剩余高度、右侧分组栏按内容高度显示。
- 修复候选输入样式来源与候选区高度，符号面板和纯画布预览不再错位。
- 修复交互预览长按退格清空，并避免拖动时误触发。
- 源码工作区改为后台初始化，过滤自身写回通知并减少频繁落盘。
- Windows 11 使用 Mica，旧版 Windows 自动回退 Acrylic。
