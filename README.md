...
- 修复画布裁剪宽高比、面板空白裁剪与 0.6.1 皮肤预览缩放问题。
- 修复 Tauri 窗口拖拽导致画布尺寸失真，以及图片/颜色选择器被置为模态的问题。
- 修复切片预览与源选区对齐、辅助线隐藏时切片绘制与选区失活的问题。
- 修复图片选择器跨归档未清空、候选预览几何未保留的问题。
- 新增可配置的面板复制工作流，支持列出目标面板 INI。
- 新增内置皮肤下载，预览新增文本类 Android 前景渲染，仅画布设备屏幕自然扩展。
