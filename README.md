...
桌面应用可在“设置 → 大模型”中配置模型服务，然后从右侧“AI 设计”描述希望修复或调整的视觉效果。AI 代理由 pi 驱动，只能调用编辑器提供的项目检查、文件列表、配置读取、INI 键设置/删除和 BDA JSON 精确替换接口；它没有 Shell、通用文件系统、文件创建/删除或二进制资源写入权限。

模型产生的内容先保存在内存草稿中。编辑器会在提交前再次检查文件版本和格式，BDA 配置还会经过 protobuf 写回验证；整轮修改只生成一个历史记录，可用一次“撤销”完整恢复。取消、模型报错或任一文件校验失败时不会应用草稿。
...
需要 Node.js 22.19+ 或 23.6+（`verify:*` 脚本直接以 `node` 运行 `.ts`，依赖默认启用的 type-stripping）、Rust stable，以及对应平台的 Tauri 2 系统依赖。
...
除 AI 设计外，编辑器在本地处理皮肤文件，不上传皮肤、配置或图片。使用 AI 设计时，模型主动读取的已授权配置文本和用户输入会发送到用户在设置中配置的模型服务；图片和其他二进制资源不会发送。API 密钥以权限受限的 JSON 文件保存在本机应用配置目录中，请只使用可信的模型服务与 API 地址。
